...
list<point> trouver_chemin(point A, point B, case_t carte[5][5])
{
    list<point> chemin;
...
void ajouter_cases_adjacentes(pair <int, int>& n, case_t carte[5][5]){
    noeud tmp;
    ...
    for (int i=0;i<4;i++)
    {
        if (cases_adjacentes[i].x<0 || cases_adjacentes[i].x>4)
            continue;
        if (cases_adjacentes[i].y<0 || cases_adjacentes[i].y>4)
            continue;
        if (carte[cases_adjacentes[i].x][cases_adjacentes[i].y]==1)
            continue;
